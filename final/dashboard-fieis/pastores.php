...
<section id="pastoras" class="d-flex flex-wrap mx-1 mt-4">
    <?php
		...
		if (count($res) > 0) {
			for ($i = 0; $i < count($res); $i++) {
				...
				$id_insersor = $res[$i]['id_insersor_ras'];
				$perfil_pas = $res[$i]['perfil_pas_ras'];
				$bio_pas = $res[$i]['bio_pas_ras'];
				...
				?>
					<div class="card mx-1" style="width: 18rem;">
					 	...
					 	<div class="card-body">
					    	<h5 class="card-title" style="font-size: 20px; font-weight: 700; margin-bottom: 2px" font-weight: 700;"><?=$nome_pas?></h5>
					    	<p class="card-text"><?=$bio_pas?></p>
	    					<a href="<?=URL_BASE."/dashboard-fieis/perfil-pastor/index.php?view_pas=".$id_pas?>"><img src="<?=IMAGEM."/fotos-pastores/$perfil_pas"?>" width="30" height="30" alt="Foto de Perfil do Pastor" style="border-radius: 100%";/></a>
					  	</div>
					</div>
				<?php
			}
...
<div class="modal" id="ModalCadPastores" data-bs-backdrop="static" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Pastores/Pastoras</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                <?php
            ...
            <form action="" method="POST" id="FormCadastrarPastor">
                <div class="modal-body">
                    ...
                    <div class="row mt-1">
                        <div class="col">
                            <select name="escolha" id="escolha" class="form-select">
