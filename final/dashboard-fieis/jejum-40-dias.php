...
<div class="ml-3 mt-3">
    <h1 class="f-family-Lobster" style="font-size: 32px">O jejum do Senhor Jesus no deserto - 40 DIAS </h1>
</div>
...
<div class="d-flex flex-wrap">
        <?php
            $query = $pdo->query("SELECT * FROM jejuns WHERE id_jejum = '6'");
    ...
    <div class="modal-dialog  modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="d-flex flex-wrap">
                    <?php
                        $query = $pdo->query("SELECT * FROM participando_do_jejum as pj JOIN usuarios as u ON pj.id_participante = u.id WHERE id_jejum_part = '$id_jejum';");
                        $res = $query->fetchAll(PDO::FETCH_ASSOC);
                        if (count($res) > 0) {
                            for ($i = 0; $i < count($res); $i++) {
                                foreach ($res[$i] as $key => $value) {
